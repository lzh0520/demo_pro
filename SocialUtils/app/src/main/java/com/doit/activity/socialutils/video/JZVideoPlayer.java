package com.doit.activity.socialutils.video;
...
public abstract class JZVideoPlayer extends FrameLayout implements
	...
	// ActivityInfo.SCREEN_ORIENTATION_SENSOR;//点击全屏按钮，先覆盖页面，横置手机时全屏
	public static int FULLSCREEN_ORIENTATION = ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE;// 点击全屏按钮，强制横屏
			...
			case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
				try {
					if (JZVideoPlayerManager.getCurrentJzvd().currentState == JZVideoPlayer.CURRENT_STATE_PLAYING) {
						JZVideoPlayerManager.getCurrentJzvd().startButton
								.performClick();
					}
	...
	public static void startFullscreen(Context context, Class _class,
                                       Object[] dataSourceObjects, int defaultUrlMapIndex,
                                       Object... objects) {
		...
		try {
			Constructor<JZVideoPlayer> constructor = _class
					.getConstructor(Context.class);
			final JZVideoPlayer jzVideoPlayer = constructor
					.newInstance(context);
			...
			jzVideoPlayer.setUp(dataSourceObjects, defaultUrlMapIndex,
					JZVideoPlayerStandard.SCREEN_WINDOW_FULLSCREEN, objects);
	...
	public static boolean backPress() {
		...
		if (JZVideoPlayerManager.getSecondFloor() != null) {
			CLICK_QUIT_FULLSCREEN_TIME = System.currentTimeMillis();
			if (JZUtils.dataSourceObjectsContainsUri(
					JZVideoPlayerManager.getFirstFloor().dataSourceObjects,
					JZMediaManager.getCurrentDataSource())) {
				JZVideoPlayer jzVideoPlayer = JZVideoPlayerManager
						.getSecondFloor();
				jzVideoPlayer
						.onEvent(jzVideoPlayer.currentScreen == JZVideoPlayerStandard.SCREEN_WINDOW_FULLSCREEN ? JZUserAction.ON_QUIT_FULLSCREEN
			...
			return true;
				...
				&& (JZVideoPlayerManager.getFirstFloor().currentScreen == SCREEN_WINDOW_FULLSCREEN || JZVideoPlayerManager
	...
	public static void showSupportActionBar(Context context) {
		if (ACTION_BAR_EXIST && JZUtils.getAppCompActivity(context) != null) {
			ActionBar ab = JZUtils.getAppCompActivity(context)
					.getSupportActionBar();
			if (ab != null) {
		...
		if (TOOL_BAR_EXIST) {
			JZUtils.getWindow(context).clearFlags(
	...
	public static void hideSupportActionBar(Context context) {
		...
		if (TOOL_BAR_EXIST) {
			JZUtils.getWindow(context).setFlags(
					WindowManager.LayoutParams.FLAG_FULLSCREEN,
					WindowManager.LayoutParams.FLAG_FULLSCREEN);
		}
	}
	...
	public static void onScrollReleaseAllVideos(AbsListView view,
		...
		int lastVisibleItem = firstVisibleItem + visibleItemCount;
		int currentPlayPosition = JZMediaManager.instance().positionInList;
		Log.e(TAG, "onScrollReleaseAllVideos: " + currentPlayPosition + " "
				+ firstVisibleItem + " " + currentPlayPosition + " "
				+ lastVisibleItem);
		if (currentPlayPosition >= 0) {
			if ((currentPlayPosition < firstVisibleItem || currentPlayPosition > (lastVisibleItem - 1))) {
	...
	public static void onChildViewAttachedToWindow(View view, int jzvdId) {
		if (JZVideoPlayerManager.getCurrentJzvd() != null
				&& JZVideoPlayerManager.getCurrentJzvd().currentScreen == JZVideoPlayer.SCREEN_WINDOW_TINY) {
			JZVideoPlayer videoPlayer = (JZVideoPlayer) view
					.findViewById(jzvdId);
			if (videoPlayer != null
					&& JZUtils.getCurrentFromDataSource(
	...
	public Object getCurrentUrl() {
		return JZUtils.getCurrentFromDataSource(dataSourceObjects,
				currentUrlMapIndex);
	}
	...
	public void init(Context context) {
		...
		fullscreenButton = (ImageView) findViewById(R.id.img_fullscreen);
	...
	public void setUp(Object[] dataSourceObjects, int defaultUrlMapIndex,
                      int screen, Object... objects) {
		if (this.dataSourceObjects != null
				&& JZUtils.getCurrentFromDataSource(dataSourceObjects,
						currentUrlMapIndex) != null
				&& JZUtils.getCurrentFromDataSource(this.dataSourceObjects,
						currentUrlMapIndex).equals(
						JZUtils.getCurrentFromDataSource(dataSourceObjects,
								currentUrlMapIndex))) {
			return;
		}
		if (isCurrentJZVD()
				&& JZUtils.dataSourceObjectsContainsUri(dataSourceObjects,
						JZMediaManager.getCurrentDataSource())) {
			long position = 0;
			try {
			...
			if (position != 0) {
				JZUtils.saveProgress(getContext(),
						JZMediaManager.getCurrentDataSource(), position);
			}
			JZMediaManager.instance().releaseMediaPlayer();
	...
	public void onClick(View v) {
		...
		if (i == R.id.start) {
			...
			if (currentState == CURRENT_STATE_NORMAL) {
				if (!JZUtils
						.getCurrentFromDataSource(dataSourceObjects,
								currentUrlMapIndex).toString()
						.startsWith("file")
						&& !JZUtils
								.getCurrentFromDataSource(dataSourceObjects,
										currentUrlMapIndex).toString()
								.startsWith("/")
						&& !JZUtils.isWifiConnected(getContext())
		...
		if (id == R.id.surface_container) {
			...
			case MotionEvent.ACTION_MOVE:
						...
						if (absDeltaX > THRESHOLD || absDeltaY > THRESHOLD) {
							...
							if (absDeltaX >= THRESHOLD) {
								...
								if (mDownX < mScreenWidth * 0.5f) {// 左侧改变亮度
									...
									WindowManager.LayoutParams lp = JZUtils
				...
				if (mChangePosition) {
					...
					mSeekTimePosition = (int) (mGestureDownPosition + deltaX
							* totalTimeDuration / mScreenWidth);
				...
				if (mChangeVolume) {
					...
					// dialog中显示百分比
					int volumePercent = (int) (mGestureDownVolume * 100 / max + deltaY
							* 3 * 100 / mScreenHeight);
				...
				if (mChangeBrightness) {
					...
					WindowManager.LayoutParams params = JZUtils.getWindow(
					...
					// dialog中显示百分比
					int brightnessPercent = (int) (mGestureDownBrightness * 100 / 255 + deltaY
							* 3 * 100 / mScreenHeight);
			...
			case MotionEvent.ACTION_UP:
				Log.i(TAG,
						"onTouch surfaceContainer actionUp [" + this.hashCode()
								+ "] ");
	...
	public void startVideo() {
		...
		JZUtils.scanForActivity(getContext()).getWindow()
	...
	public void onStatePrepared() {// 因为这个紧接着就会进入播放状态，所以不设置state
		if (seekToInAdvance != 0) {
			...
			long position = JZUtils.getSavedProgress(getContext(), JZUtils
					.getCurrentFromDataSource(dataSourceObjects,
							currentUrlMapIndex));
			if (position != 0) {
	...
	public void onError(int what, int extra) {
		...
		Log.e(TAG, "错误码--" + this.hashCode());

		if (what != 38 && extra != -38 && what != -38 && extra != 38
				&& extra != -19) {
			onStateError();
			if (isCurrentPlay()) {
				JZMediaManager.instance().releaseMediaPlayer();
	...
	public void onAutoCompletion() {
		...
		JZMediaManager.instance().releaseMediaPlayer();
		JZUtils.saveProgress(getContext(), JZUtils.getCurrentFromDataSource(
				dataSourceObjects, currentUrlMapIndex), 0);
	}
	...
	public void onCompletion() {
		...
		if (currentState == CURRENT_STATE_PLAYING
			...
			JZUtils.saveProgress(getContext(), JZUtils
					.getCurrentFromDataSource(dataSourceObjects,
							currentUrlMapIndex), position);
		}
		cancelProgressTimer();
		onStateNormal();
	...
	public void release() {
		if (JZUtils.getCurrentFromDataSource(dataSourceObjects,
				currentUrlMapIndex).equals(
				JZMediaManager.getCurrentDataSource())
			...
			} else if (JZVideoPlayerManager.getSecondFloor() == null
					&& JZVideoPlayerManager.getFirstFloor() != null
	...
	public void initTextureView() {
		...
		JZMediaManager.textureView.setSurfaceTextureListener(JZMediaManager
				.instance());
	}
	...
	public void removeTextureView() {
		JZMediaManager.savedSurfaceTexture = null;
		if (JZMediaManager.textureView != null
				&& JZMediaManager.textureView.getParent() != null) {
			((ViewGroup) JZMediaManager.textureView.getParent())
					.removeView(JZMediaManager.textureView);
		}
	}
	...
	public void onVideoSizeChanged() {
		...
		if (JZMediaManager.textureView != null) {
			...
			JZMediaManager.textureView.setVideoSize(
	...
	public void setProgressAndText(int progress, long position, long duration) {
		...
		if (position != 0)
			currentTimeTextView.setText(JZUtils.stringForTime(position));// 当前时间
		totalTimeTextView.setText(JZUtils.stringForTime(duration - position));// 视频剩余时间
		// totalTimeTextView.setText(JZUtils.stringForTime(duration));//视频总时长

		// 到课程收费时设置登录提示，目前都是免费课程，不需要
		// if ("00:05".equals(JZUtils.stringForTime(position))
	...
	public void onStopTrackingTouch(SeekBar seekBar) {
		...
		JZMediaManager.seekTo(time);
		Log.i("data", "时间-----" + mSeekTimePosition + "?=" + time + "--进度----"
				+ seekBar.getProgress());
	...
	public void startWindowFullscreen() {
		...
		textureViewContainer.removeView(JZMediaManager.textureView);
		try {
			Constructor<JZVideoPlayer> constructor = (Constructor<JZVideoPlayer>) JZVideoPlayer.this
			...
			jzVideoPlayer.setUp(dataSourceObjects, currentUrlMapIndex,
					JZVideoPlayerStandard.SCREEN_WINDOW_FULLSCREEN, objects);
			...
			// jzVideoPlayer.setAnimation(ra);
			onStateNormal();
			jzVideoPlayer.progressBar.setSecondaryProgress(progressBar
	...
	public void startWindowTiny() {
		...
		try {
			...
			vp.addView(jzVideoPlayer, lp);
			jzVideoPlayer.setUp(dataSourceObjects, currentUrlMapIndex,
					JZVideoPlayerStandard.SCREEN_WINDOW_TINY, objects);
			jzVideoPlayer.setState(currentState);
			jzVideoPlayer.addTextureView();
			JZVideoPlayerManager.setSecondFloor(jzVideoPlayer);
			onStateNormal();
	...
	public boolean isCurrentPlay() {
		return isCurrentJZVD()
				&& JZUtils.dataSourceObjectsContainsUri(dataSourceObjects,
						JZMediaManager.getCurrentDataSource());// 不仅正在播放的url不能一样，并且各个清晰度也不能一样
	}
	...
	public void autoFullscreen(float x) {
		if (isCurrentPlay() && currentState == CURRENT_STATE_PLAYING
				...
				&& currentScreen != SCREEN_WINDOW_TINY) {
			if (x > 0) {
				JZUtils.setRequestedOrientation(getContext(),
	...
	public void onEvent(int type) {
		if (JZ_USER_EVENT != null && isCurrentPlay()
				&& dataSourceObjects != null) {
			JZ_USER_EVENT.onEvent(type, JZUtils.getCurrentFromDataSource(
					dataSourceObjects, currentUrlMapIndex), currentScreen,
					objects);
		}
	}
	...
	public void showProgressDialog(float deltaX, String seekTime,
                                   long seekTimePosition, String totalTime, long totalTimeDuration) {
	}
			...
			if (currentState == CURRENT_STATE_PLAYING
				...
				mHandler.post(new Runnable() {
					...
					public void run() {
						long position = getCurrentPositionWhenPlaying();
						long duration = getDuration();
						int progress = (int) (position * 100 / (duration == 0 ? 1
								: duration));
